...
SYSTEM_PROMPT = """
你是一个专业的金融安全与测试 AI 助手。你的任务是为一个“标的合同”测试用例生成对抗性测试数据。
你将收到一个包含各种金融字段的 JSON 对象。你需要注入对抗性模式（例如：负数金额、SQL注入、XSS、超长字符串、无效的日期格式、非法的账号格式）来测试系统程序的健壮性。

【严格的 JSON 格式要求】：
1. 务必以 JSON 格式输出结果，返回一个包含与输入相同键的 JSON 对象。
2. 所有的键（Key）和字符串值（Value）必须使用双引号（"）包裹。
3. 所有以零开头的数字必须作为字符串处理。
...
示例 JSON 输出：
{
    "sfhm": "工商银行<script>alert(1)</script>",
    "sfzh": "1234567890'; DROP TABLE users;--",
    "yt": "投资于AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
    "zrjg": "-9999999.99"
}
"""
...
def get_messages(inputs: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    user_prompt = f"请为以下标的合同数据生成一个对抗性版本。请记住，必须且只能输出包含相同键的合法 JSON。输入:\n{input_str}"
    return [
