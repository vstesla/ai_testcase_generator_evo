...
class ProcessService:
    ...
    async def process_ckxy(self, business_process: str, count: int, file_path_identifier: str, enable_comparison: bool = True) -> Dict[str, Any]:
        ...
        if enable_comparison:
            await self._parse_and_evaluate(test_case_id_batch, generated_file_info)
        ...
        if not self._is_local_ocr_enabled():
            try:
                remote_pool = ConnectionPool(
                    host=os.getenv("REMOTE_PARSE_DB_HOST", ""),
                    port=int(os.getenv("REMOTE_PARSE_DB_PORT", "3306")),
                    user=os.getenv("REMOTE_PARSE_DB_USER", ""),
                    password=os.getenv("REMOTE_PARSE_DB_PASSWORD", ""),
